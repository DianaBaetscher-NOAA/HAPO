...
# output for jamie
sample_tbl %>%
  write_csv("csv_outputs/microkit_sample_asv_quicklook.csv")
...
sample_tbl %>% 
 # left_join(., tax_df, by = "asv") %>%
  filter(reads > 10) %>%
  ggplot(aes(x = reorder(sample, reads), y = reads, fill = asv)) +
...
sample_tbl %>% 
 # left_join(., tax_df, by = "asv") %>%
  ...
  ggplot(aes(x = reorder(sample, tot_reads), y = reads, fill = asv)) +
  geom_bar(stat = "identity") +
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 90, size = 7)
