...
out <- filterAndTrim(fnFs, filtFs, truncLen = 250,
...
```{r regseqs-asv-output}
 #make fasta file with ASVs
    ...
    {
        write.table(paste(">ASV",i, sep=""),file="csv_outputs/HAPO_micro_20260324_ASV_fwd250bp.csv", append=TRUE, col.names = F, row.names = F, quote=F)
        write.table(paste(asv_seqs[i], sep=""),file="csv_outputs/HAPO_micro_20260324_ASV_fwd250bp.csv", append=TRUE, col.names = F, row.names = F, quote=F)
    }
```
