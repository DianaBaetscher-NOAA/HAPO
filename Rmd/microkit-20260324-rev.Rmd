...
The forward primers are analyzed in `microkit-20260324.Rmd`

Here, I'll look at the reverse primers to see if those show up.
...
for i in ${NAMELIST}; do
	cutadapt --discard-untrimmed -a ATGGTCCTGAAGTAAGAACCAGATG -o trimmed_rev/${i}_R1.fastq.gz "$DATA/${i}_L001_R1_001.fastq.gz" >> cutadapt_rev_out.txt
	done


# then unzip the reads in the /trimmed_rev directory
pigz -d *gz
```


~45-50% of reads have the reverse primer sequence. But how? If 99% of reads have the forward primer sequence?
...
```{r}
# file location
path <- "../data/20260324_HAPOeDNA/trimmed_rev"
...
# move around empty files
# after doing this, I go back up to list the files because the file.list cannot include empty files
# having commented out this section.
...
# dir.create("../data/20260324_HAPOeDNA/trimmed_rev_empty", showWarnings = FALSE)
# 
# # move the files
# file.rename(empty_files,
#             file.path("../data/20260324_HAPOeDNA/trimmed_rev_empty", basename(empty_files)))
...
All sequences were trimmed to 275 bp per our initial trimming step.
...
```{r regseqs-asv-output}
 #make fasta file with ASVs
    ...
    {
        write.table(paste(">ASV",i, sep=""),file="csv_outputs/HAPO_micro_20260324_ASV_rev_275bp.csv", append=TRUE, col.names = F, row.names = F, quote=F)
        write.table(paste(asv_seqs[i], sep=""),file="csv_outputs/HAPO_micro_20260324_ASV_rev_275bp.csv", append=TRUE, col.names = F, row.names = F, quote=F)
    }
```
...
write.csv(st.brief, file="csv_outputs/microkit_20260324_rev_sampleTable.csv")
